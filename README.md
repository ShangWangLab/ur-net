...
Implementation and training data for OCT embryonic mouse yolk sac angiography and endocardium segmentation with Pico-Net and PicoR-Net, a small 3D CNN which can identify blood flows and endocardium even with minimal presence of blood cells.
...
* See [trained_models](trained_models) for the pretrained models used in the referenced paper; [Pico-Net](trained_models/pico-net_1600.pt) is the PyTorch state dictionary for the base model, and [PicoR-Net](trained_models/picor-net_b0.5_1200.pt) is likewise for the refinement version trained with $\beta^2 = 0.5$.
...
In the command line, run: `python train_PicoNet.py`
...
If you would like to subsequently train a PicoR-Net, enter `bootstrap` when the training completes to generate a PicoR-Net with the same parameters as the latest Pico-Net model and zero weights for the refinement input layer.

# Training PicoR-Net

Typically, PicoR-Net is trained starting from a pretrained Pico-Net. If you have not already in the previous step, bootstrap this initial version of PicoR-Net in a command line by running: `python bootstrap_PicoRNet.py` The default will bootstrap using the pretrained Pico-Net from [trained_models](trained_models). Modify the input path if you desire a different input.

To train the bootstrapped model, run: `python train_PicoRNet.py`. It may take a while to generate the [size map cache](size_map_cache) the first time it is run.
...
During training, you can type `val` or `test` to evaluate the model on the validation (03_T17) or test (07_T40) volume, respectively, generating a binary mask. When training a PicoR-Net, refinement will be performed over 10 iterations starting from a blank mask of zeros.
